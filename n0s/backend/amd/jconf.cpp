...
bool jconf::GetThreadConfig(size_t id, thd_cfg& cfg)
{
	...
	const Value *idx, *intensity, *w_size, *aff, *unroll, *compMode, *interleave;
	...
	if(idx == nullptr || intensity == nullptr || w_size == nullptr || aff == nullptr ||
		unroll == nullptr || compMode == nullptr)
		return false;
...
bool jconf::parse_config(const char* sFilename)
{
	...
	for(size_t i = 0; i < n_thd; i++)
	{
		...
		{
			printer::inst()->print_msg(L0, "Thread %zu has invalid config.", i);
			return false;
		}
	}
	return true;
