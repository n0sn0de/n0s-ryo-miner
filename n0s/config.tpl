...
/*
 * LARGE PAGE SUPPORT
 * Large pages and memory locking are optional startup optimizations for the miner's CPU-side scratchpad buffers.
 * They DO NOT change the GPU kernels or make the miner unusable when unavailable.
 *
 * On this GPU-focused miner, the practical impact of falling back to standard pages is usually small. The default
 * "warn" mode is therefore the sane choice on most rigs: try the optimization, but keep mining if the OS or user
 * session is not configured for it.
 *
 ...
 *---LINUX
 * Memory locking means that the kernel can't swap out the page to disk. On a healthy dedicated mining box with---LINUX
 * enough RAM, the effect is usually minor. If you want huge pages without trying to mlock, use "no_mlck".---LINUX
 *
 *
 * use_slow_memory defines our behaviour with regards to large pages. There are three possible options here:
 * always  - Don't even try to use large pages. Always use slow memory.
 * warn    - Try large pages first, print a notice if unavailable, then continue with standard pages.
 * no_mlck - This option is only relevant on Linux, where we can use large pages without locking memory.---LINUX
 *           It requires huge pages, but it won't attempt to mlock---LINUX
 * never   - Require huge/large pages. If we cannot allocate them, print an error and exit.
 */
