#!/bin/bash
# Native remote CUDA validation helper.
# Default mode is benchmark because it is more reproducible than share acceptance.
set -e

REMOTE="${REMOTE:-cuda-test-host}"
REPO_URL="${REPO_URL:-https://github.com/n0sn0de/n0s-ryo-miner.git}"
REMOTE_DIR="${REMOTE_DIR:-/home/${REMOTE}/n0s-ryo-miner}"
...
echo "====================================="
echo "Remote Native CUDA Validation"
echo "Remote: ${REMOTE}"
...
ssh "${REMOTE}" "set -e && \
  ...
  cmake .. -DCUDA_ENABLE=ON -DOpenCL_ENABLE=OFF -DMICROHTTPD_ENABLE=ON \
    -DN0S_COMPILE=generic -DCUDA_ARCH='${CUDA_ARCH}' -DCMAKE_BUILD_TYPE=Release && \
  cmake --build . -j\$(nproc) && \
  test -f bin/n0s-ryo-miner"

echo "✅ Native CUDA build succeeded on ${REMOTE}"

if [ "${MODE}" = "benchmark" ]; then
  echo "Running benchmark..."
  ssh "${REMOTE}" "set -e && \
    export LD_LIBRARY_PATH=${CUDA_HOME}/lib64:\$LD_LIBRARY_PATH && \
    cd ${REMOTE_DIR} && timeout 130 ./build/bin/n0s-ryo-miner --noAMD \
      --benchmark 10 --benchmark-json ${REMOTE_DIR}/benchmark.json >/dev/null 2>&1 || true && \
    cat ${REMOTE_DIR}/benchmark.json"
  exit 0
fi

echo "Mining for ${TIMEOUT} seconds..."
OUTPUT=$(ssh "${REMOTE}" "export LD_LIBRARY_PATH=${CUDA_HOME}/lib64:\$LD_LIBRARY_PATH && \
